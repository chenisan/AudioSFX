...
3. **生成音效**（三種入口）：
   - **文字 → SFX**：頂部〰「音效」浮窗 → 英文 prompt（如 `glass shatter`）→ 生成。
...
3. **Generate SFX** (three entry points):
   - **Text → SFX**: top 〰 "音效" floating window → English prompt (e.g. `glass shatter`) → generate.
